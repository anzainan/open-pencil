...
export class BridgeClient {
    ...
    async listFiles() {
        ...
        if (!response.ok)
            throw new Error(`Bridge list files failed (${response.status})`);
        return (await response.json());
    }
    async getFile(path) {
        const response = await fetch(`${this.apiBase}/files/${encodeRelPath(path)}`);
        if (!response.ok)
            throw new Error(`Bridge read failed (${response.status}): ${path}`);
    ...
    async createFile(brand, name, format = 'fig') {
        const response = await fetch(`${this.apiBase}/files`, {
            method: 'POST',
        ...
        if (!response.ok)
            throw new Error(`Bridge create failed (${response.status})`);
        const data = (await response.json());
        if (!data.path)
            throw new Error('Bridge create returned no path');
        return data.path;
    }
    async deleteFile(path) {
        ...
        if (!response.ok)
            throw new Error(`Bridge delete failed (${response.status}): ${path}`);
    }
    /** 上报活动文件（打开/切换 tab 时）。失败仅警告，不影响编辑。 */
    async reportActive(path) {
        await this.postState('/active', path);
    }
    /** 标记为「打开过」，供列表页最近打开排序。 */
    async reportRecent(path) {
        await this.postState('/recent', path);
    }
    async postState(endpoint, path) {
        try {
    ...
    ensureConnection() {
        ...
        const handle = (event) => {
            ...
            if (Number.isFinite(seq) && seq > 0 && this.seenSeqs.has(seq))
                return;
    ...
    isThrottled(path) {
        ...
        if (now - last < PATH_THROTTLE_MS)
            return true;
        if (this.lastHandledAt.size > 100)
            this.lastHandledAt.clear();
     ...
     */
    watchPath(path, getLastWriteTime, reloadFromDisk) {
        this.ensureConnection();
        let lastMtime = '';
        ...
        const handleEvent = (event) => {
            ...
            if (event.path !== path)
                return;
            if (Date.now() - getLastWriteTime() < this.recentWriteMs)
                return;
            if (this.isThrottled(path))
                return;
            reloadFromDisk();
        };
